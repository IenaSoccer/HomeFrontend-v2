  ...
  <div class="" v-cloak v-if="loaded">
    <div class="q-gutter-md" v-if="data && data.id">
      <q-carousel
        ...
        arrows
        height="600px"
        class="bg-dark text-[#f3f4f6]"
      >
        <q-carousel-slide
          v-for="s in data.content.data"
          :key="s.id"
          :name="s.id"
          class="p-0"
        ...
        >
          <q-video
            v-if="s.data.path ? s.data.path.endsWith('.mp4') : false"
          ...
          <div class="flex flex-col justify-between text-3xl roboto-flex h-full">
            <div
              class="z-10 bg-[rgba(0,0,0,.6)] h-16 w-full flex flex-row justify-start items-center px-2"
            >
              <div class="flex flex-row flex-wrap justify-between items-center w-full">
                ...
                <q-btn
                  ...
                  rounded
                  flat
                  @click="downloadMedia(s.data.path)"
                ></q-btn>
              </div>
            </div>
            <div
              class="z-10 bg-primary opacity-70 my-12 self-end max-w-1/2 flex flex-row justify-center items-center"
            >
              <div class="flex flex-row justify-center items-center w-full">
                <q-btn
                  ...
                  :label="'#' + tag"
                  flat
                  v-for="(tag, i) in data.attributes?.tags"
                  :key="i"
