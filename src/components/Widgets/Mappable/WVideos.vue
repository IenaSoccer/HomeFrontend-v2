      ...
      <q-carousel class="w-[700px]" swipeable animated v-model="slide" infinite>
        <template #control>
          <div class="absolute bottom-0 left-0 right-0 flex justify-between px-4 z-5">
            <q-btn round dense size="xl" flat icon="navigate_before"
              @click="slide = (data[0]?.content.data?.length) ? (slide - 1 + data[0]?.content.data!.length) % data[0]?.content?.data.length : 0"
              class="text-white" />
            <q-btn round dense size="xl" flat icon="navigate_next"
              @click="slide = (data[0]?.content.data?.length) ? (slide + 1) % data[0]?.content.data!.length : 0"
              class="text-white" />
          </div>
        </template>
        <q-carousel-slide v-for="(single, k) in (data[0]?.content?.data as Attachment[])" :key="single.id"
          class="bg-dark" style="padding: 0 !important;" :name="k">
  ...
  watch: {
    videos: {
      handler() {
        ...
        Promise.all(this.videos.map(async (single) => {
