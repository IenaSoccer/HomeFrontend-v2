...
type Extractable = Attachment | Folder | Folder[] | Resource<UUID, UUID[]> | Resource<UUID, UUID[]>[] | Alert[] | Mappings;
...
const extractThumbnails = async (thumbs: UUID | UUID[], store: ResourceStore): Promise<Attachment[]> =>
  extract(Array.isArray(thumbs) ? thumbs : [thumbs], store);
