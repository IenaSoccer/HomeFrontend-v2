  ...
  watch: {
    mappings: {
      ...
      async handler(n: Mappings, o: Mappings) {
        ...
        this.resources['galleria'] = await this.ensureResource(n.defaults['galleria'].id, 'galleria');

        if (this.$route.params.uuid)
          this.selectedAlbum = this.resources['galleria'].find(r => r.id === this.$route.params.uuid) as Resource<UUID>;
